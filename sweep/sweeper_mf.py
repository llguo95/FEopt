...
def objective_function(x: np.ndarray) -> np.ndarray:
    ...
    cmdl = '"ansys2019r3" -p ansys -dis -mpi INTELMPI -np 2 -lch -dir "wirebond_mf_sweep" -j "wirebond_mf" -s read -l en-us -b -i "wirebond_mf_sweep/01_Geometric_Inputs.txt"'
    os.system(cmdl)

    out_path = 'wirebond_mf_sweep/Max_Strain_' + str(x[0]) + '_' + str(x[1]) + '.txt'
