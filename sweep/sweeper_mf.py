...
def objective_function(x: np.ndarray, MeshSize_XY: float) -> np.ndarray:
    ...
    work_folder_name = 'wirebond_mf_sweep_' + str(MeshSize_XY)

    if not os.path.exists(work_folder_name):
        os.mkdir(work_folder_name)
        copy_tree("wirebond_mf_sweep", work_folder_name)

    in_content = open(work_folder_name + '/01_Geometric_Inputs_original.txt', 'r').read()
    new_in_content = 'WThk = ' + str(x[0]) + '\n\nFL = ' + str(x[1]) + '\n\n' + \
                     in_content[in_content.index("!Loop Heights: (Limit 0.6 to 2.3)"):]
    text_file = open(work_folder_name + '/01_Geometric_Inputs.txt', 'w')
    text_file.write(new_in_content)
    text_file.close()

    in_content_lf = open(work_folder_name + '/02_Mesh_Parameters_original.txt', 'r').read()
    ...
    text_file_lf = open(work_folder_name + '/02_Mesh_Parameters.txt', 'w')
    ...
    out_path = work_folder_name + '/Max_Strain_' + str(MeshSize_XY) + '_' + str(x[0]) + '_' + str(x[1]) + '.txt'
...
meshsizes = [0.5, 0.75, 1.0, 1.25, 1.5]
